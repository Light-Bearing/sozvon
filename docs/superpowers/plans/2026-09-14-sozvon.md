...
> **Состояние на 24.09.2026.** Задачи 1–12 выполнены и выложены; галочки по
> ходу не ставились, и задним числом я их не ставлю — пункты поштучно не
> перепроверены. Задача 13 (живой звонок двух устройств) пройдена
> 17.09.2026. Задача 14 (сторож адресов) сделана 24.09 в упрощённом виде:
> еженедельная проверка на GitHub краснеет и присылает письмо, но список
> сама не правит. Стоит на пробнике, починенном в тот же день, — прежний
> записывал живые mqtt-брокеры в мёртвые. Живое состояние проекта — в README, раздел «Состояние».
...
describe('секрет комнаты', () => {
  ...
  it('то, что вовсе не ссылка, тоже даёт null, а не исключение', () => {
...
export const linkToSecret = href => {
  // Обещано string | null, значит на кривой ссылке возвращаем null,
  // а не бросаем: вставленный руками мусор — обычное дело.
  let hash;
  try {
...
describe('адреса каналов', () => {
  ...
  it('к проверенным дописываются адреса из библиотеки', () => {
    ...
    // не входит — ровно поэтому мы дописываем, а не заменяем.
...
**Внимание: код ниже — исправленный.** Первая редакция плана велела слать
медиа во все три комнаты сразу, и это давало до трёхкратной лишней отдачи:
`addStream` без адресата обходит всех собеседников комнаты, а человек,
найденный тремя семействами, имеет три соединения. Правило теперь такое:
**решает отправитель** — своё медиа и свои сообщения шлём каждому только по
каналу его владельца, адресно; принимаем от любого семейства, повторный
поток от того же участника отбрасываем.
...
адресов по умолчанию мертва, а живой relay.snort.social в список nostr
вообще не входит.
...
**Внимание: код ниже — исправленный.** Первая редакция сливала два разных
дела в `applyStep`, и из-за этого потолок битрейта не включался вовсе при
звонке вдвоём: ступень там не меняется, а выход был по её имени. Теперь
перенастройка камеры идёт только при настоящей смене ступени, а потолок
проставляется каждый раз. Обходы собеседников защищены поштучно: сбой у
одного не должен ронять подстройку всему звонку.
...
В `applyStep` добавить решение о камере — **рядом с `applyBitrateCeiling()`,
вне проверки `if (changed)`**:
...
Порядок внутри `applyStep` должен получиться такой: перенастройка камеры
при смене ступени → потолок битрейта → решение о камере по говорящему →
перерисовка при смене ступени.
...
**Внимание: код ниже — исправленный.** Первая редакция определяла, гость
перед нами или хозяин, по тому, что осталось в хвосте адреса. Из-за этого
хозяин, которому не дали камеру, после «Попробовать снова» видел гостевое
«Вас зовут поговорить» — приложение сообщало человеку, что его зовут,
хотя звонил он сам. Роль теперь определяется один раз при загрузке, до
того как что-либо запишется в адрес. Гостю хвост не чистим никогда:
это его единственная копия ссылки.
...
### Task 14: Сторож адресов — расписание вместо памяти

Публичные адреса смертны, это записано в спеке с первого дня. Лечится
прогоном пробника — которого руками раз в неделю не будет делать никто.
Однажды приложение начнёт молча биться в мёртвые адреса, и никто не
поймёт почему.

Actions для этого — законное применение: проверка и поддержание самого
проекта, а не посторонняя служба.

Заодно тем же заходом проверяются ретрансляторы. Сегодня живых нет ни
одного (см. «Главная нерешённая задача» в спеке), но если появится —
подберётся сам.

**Files:**
- Create: `.github/workflows/relays.yml`
- Modify: `tools/probe.mjs` (режим `--json` — вывод для машины, а не для
  человека; человеческий вывод остаётся по умолчанию)
- Create: `tools/refresh-relays.mjs` (переписывает `VERIFIED` и
  `VERIFIED_ON` в `src/signal/relays.js` по итогам прогона)
- Create: `tests/refresh-relays.test.js`
- Modify: `README.md` (раздел «Проверка связи» — сказать про расписание)

**Interfaces:**
- Consumes: `tools/probe.mjs`, `src/signal/relays.js`
- Produces: изменение (pull request) с обновлённым списком адресов, если
  что-то отвалилось; тишину, если всё живо.

- [ ] **Step 1: Пробник умеет отдавать машинный вывод**

Написать провальный тест: `node tools/probe.mjs --json` печатает разбор
в JSON — по каждому адресу `{family, url, ok, ms}` и по NAT
`{verdict, symmetric}`. Внешний адрес в вывод **не попадает**: это
домашний адрес человека, и ему незачем оказываться в журнале сборки.

Реализовать. Человеческий вывод без ключа не меняется ни на строку.

- [ ] **Step 2: Обновление списка — отдельно и с тестами**

`tools/refresh-relays.mjs` берёт JSON пробника и правит `VERIFIED` в
`src/signal/relays.js`. Правила, каждое отдельным тестом:

- живой адрес, которого нет в списке, — добавляется;
- мёртвый адрес, который в списке есть, — убирается;
- **семейство не опустошается никогда**: если живых в нём не осталось,
  список этого семейства оставляем как был и помечаем в отчёте. Пустое
  семейство хуже устаревшего: библиотека в этом случае молча берёт свои
  умолчания, и мы перестаём понимать, чем вообще пользуемся;
- `VERIFIED_ON` ставится в дату прогона только если что-то изменилось.

- [ ] **Step 3: Расписание**

`.github/workflows/relays.yml`: `schedule` раз в неделю плюс
`workflow_dispatch`. Прогон, обновление, и если файл изменился —
`peter-evans/create-pull-request`. **Не пуш в main напрямую**: смена
адресов связи — это то, на что стоит посмотреть глазами, а не узнать
из журнала выкладки.

Проверить руками через `workflow_dispatch` до того, как полагаться на
расписание: невыполнявшаяся ветка содержит ошибку чаще, чем кажется.

- [ ] **Step 4: Ретрансляторы в том же прогоне**

Дописать в пробник проверку TURN: выделяется ли адрес `relay`. Без
браузера это делается сырым запросом Allocate по TURN — или, если проще
и надёжнее, отдельным шагом на `chrome-headless-shell` с
`iceTransportPolicy: 'relay'`.

Список кандидатов и их пропуска держим **в файле рядом с пробником, а не
в коде приложения**: это данные для проверки, а не настройка звонка. В
`relays.js` ничего из них не попадает, пока человек не решил (см. спеку).

- [ ] **Step 5: Записать в README**

В разделе «Проверка связи» сказать: списки адресов проверяются
расписанием раз в неделю, изменения приезжают отдельным предложением,
а не втихую.
...
```bash
git add -A
git commit -m "Адреса каналов проверяет расписание, а не память

Публичные адреса смертны, а прогонять пробник руками раз в неделю не
будет никто. Раз в неделю Actions проверяет их сам и открывает
предложение с заменой — в main напрямую не пишет: смена адресов связи
стоит того, чтобы на неё посмотрели.

Семейство не опустошается никогда: без адресов библиотека молча берёт
свои умолчания, и мы перестаём понимать, чем пользуемся.
...
- **Ретранслятор.** Из отложенной мелочи он вырос в **главную нерешённую
  задачу** — ей отведён отдельный раздел в спеке. Коротко: бесплатных без
  регистрации в мире не осталось (проверено 15.09.2026 живым Chrome),
  IPv6 у нас наружу не проходит, а в браузере закрыты все приёмы, которыми
  обходят NAT настоящие P2P-системы. Каждый работающий вариант нарушает
  либо «ничего не платить и не чинить», либо «перешёл по ссылке и
  говоришь». Выбор за человеком, и он не сделан.

  Самый близкий к исходному замыслу — **ретранслятор из самого
  участника**: если трое и двое не видят друг друга, а третий видит
  обоих, пустить звук через третьего. Серверов ноль. Вдвоём не работает.
