...
Добавить кнопку в `#screen-start` и в `#screen-failed`:

```html
      <button id="diagnostics-open" class="quiet-button">Диагностика</button>
```
...
for (const button of app.querySelectorAll('#diagnostics-open')) {
  button.onclick = () => {
...
Обратите внимание: `#diagnostics-open` встречается дважды, поэтому
`querySelectorAll`, а не `querySelector`. Дублирующиеся `id` в HTML
недопустимы — второй кнопке дать `id="diagnostics-open-failed"` и
подключать обе через `app.querySelectorAll('[data-diagnostics]')`,
проставив атрибут `data-diagnostics` обеим.
