...
// У микрофона и камеры белое с перечёркиванием значит «выключено, и это
// важно». У экрана «не показываю» — обычное состояние, и значок с
// перечёркиванием был бы ложной тревогой.
describe('кнопка экрана не притворяется тревогой', () => {
  it('у неё один значок, а не пара «включено/выключено»', () => {
    const el = root();

    expect(el.querySelectorAll('#screen .i')).toHaveLength(1);
